<template>
  <div class="connectWallet">
    <button  style="margin-right: 20px;width: 120px" class="button-connect" @click="isShowSelect = true">
      {{ curChain }}
    </button>
    <div class="select-chain" v-show="isShowSelect">
      <div class="mask" @click="isShowSelect=false">

      </div>
      <div class="select-list">
        <div class="select-item" @click="chooseChain('Kovan')">
          Kovan
        </div>
        <div class="select-item" @click="chooseChain('Rinkeby')">
          Rinkeby
        </div>
        <div class="select-item" @click="chooseChain('BSCTEST')">
          BSCTEST
        </div>
        <div class="select-item" @click="chooseChain('AuroraTest')">
          AuroraTest
        </div>
        <div class="select-item" @click="chooseChain('AvalancheFuji')">
          AvalancheFuji
        </div>
      </div>
    </div>
...
import chainInfo from "../utils/chainInfo.json"
export default {
  name: "ConnectWallet",
  data() {
    return {
  ...
  methods: {
    chooseChain(chain){
      ...
      this.curChain = chain
      localStorage.setItem('curChain', chain)
      if(chainInfo[chain]){
        let chainObj = chainInfo[chain]
        if(chainObj.type == 'other'){
    ...
    async connectWallet(idx) {
      this.chooseChain(this.curChain)
      this.connectIdx = idx
      this.isLoading = true
      // window.ethereum.request({
      //   method: 'wallet_addEthereumChain',
